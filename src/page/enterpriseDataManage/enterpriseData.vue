<template>
  <div class="container">
    ...
    <div class="top" v-if="false">
      <ul class="flex-item" v-if="querySummaryData">
        ...
        <li>申请时间：{{querySummaryData.createTime}}</li>
      	...
      	<el-button :type="querySummaryData.status=='INIT'?'warning':querySummaryData.status=='EVALUATING'?'success':querySummaryData.status=='CANCELED'?'info':querySummaryData.status=='REJECT_EVALUATE'?'danger':''" size="mini">
      ...
      <div class="flex-item" style="position: relative; width: 100%;">
        ...
        <el-tabs v-model="activeTab" type="card" @tab-click="handleClick">
        		...
        		<div class="tab-content">
	        		...
	        		<el-collapse @change="changeVerifyFirmInfo" v-model="activeVerifyFirmInfoNames" class="marginBotten">
			                    ...
			                    <!--<el-date-picker
			                      ...
			                      placeholder="选择日期">
								    		...
								    		<el-form-item label="经营地面积：">
								    			<span class="input_read">{{enterpriseInfoData.locationArea}}</span>
								    		...
								    		<el-form-item label="水电月缴费用：">
								    			<span class="input_read">{{enterpriseInfoData.utilitiesMonth}}</span>
	                          ...
	                          <Uploads
	                          	...
	                          	actionUrl="/capital-api/supply_data/upload_image_file"></Uploads>
	                        </template>
								    		</el-form-item>
								    		<div style="text-align: right;" v-if="false">
        	...
        	<el-tab-pane label="个人信息" name="second">
        		<div class="tab-content">
        			<!--担保人-->
	        		<el-collapse @change="changeVerifyFirmBondsman"  v-model="activeVerifyFirmBondsmanNames"  class="marginBotten">
	        			<el-collapse-item name="FirmFinancialBondsmanOne">
	        					<template slot="title">
								        	法定代表人信息
								    </template>
								    <el-form ref="verifyFirmBondsmanForm" label-width="180px" size="mini" label-position="right">
								    	<div class="info-list">
								    		<el-form-item label="法定代表人姓名：">
		                        ...
		                        <template>
								    		...
								    		<el-form-item label="学历：">
								    			<span class="input_read">{{queryEnterpriseGuarantorData.educationBackgroundText}}</span>
	                    		<!--<template slot-scope="scope">
	                    		...
	                    		<template>
	                    			<span class="input_read">{{queryEnterpriseGuarantorData.localResidentText}}</span>
							    		...
							    		<template>
										    <el-table
										      ...
										      <el-table-column
										        ...
										        label="估值">
										        <template slot-scope="scope">
										        	<span class="table_input_read">{{scope.row.estimatedValue?scope.row.estimatedValue:'-'}} 万元</span>
										        	<!--<el-input
															  type="input"
															  v-model="scope.row.estimatedValue">
															  <template slot="append">万元</template>
										      ...
										      <el-table-column
										        ...
										        label="车贷月还">
										        <template slot-scope="scope">
										        	<span class="table_input_read">{{scope.row.paymentMonthly?scope.row.paymentMonthly:'-'}} 元</span>
										        	<!--<el-input
															  type="input"
															  v-model="scope.row.paymentMonthly">
															  <template slot="append">元</template>
										      ...
										      <el-table-column
										        ...
										        label="车辆信息">
										        <template slot-scope="scope">
										        	<span class="table_input_read">{{scope.row.assetName?scope.row.assetName:'-'}}</span>
										        	<!--<el-input
															  type="textarea"
															  v-model="scope.row.assetName">
															</el-input>-->
								    ...
								    <el-form ref="verifyAssetInfoForm" label-width="170px" size="mini" label-position="right">
								    	<div class="info-list">
								    		...
								    		<el-form-item label="基本组织架构：">
								    			<el-input type="textarea" :rows="4" :disabled="true" v-model="companyManageData.organizationStructure">
	                				</el-input>
								    		</el-form-item>
        			...
        			<el-form ref="verifyAssetInfoForm" label-width="170px" size="mini" label-position="right">
        				<div class="info-list">
					    		...
					    		<el-form-item label="商家编码：" >
					    			<span class="input_read">{{merchantCode}}</span>
	                  ...
	                  <el-button  v-if="!roleHasPermissions['supplier_manage_maintain_list_save']" @click="saveQueryTitanUpdownstream" type="primary">保存</el-button>
					    	...
					    	<div class="info-list">
					    		...
					    		<template>
									  <el-table
									    :data="titanUpdownstreamData.upStream"
									    border
									    style="width: 100%">
									    <el-table-column
									      prop="date"
					    	...
					    	<div class="info-list">
					    		...
					    		<template>
									  <el-table
									    :data="titanUpdownstreamData.downStream"
									    border
									    style="width: 100%">
									   <el-table-column
									      prop="date"
    			 	 	...
    			 	 	<el-form ref="verifyFirmInfoForm" label-width="170px" size="mini" label-position="right">
					    	<div class="info-list">
					    		<el-form-item label="申请额度：">
					    			<span class="input_read">{{borrowInfoData.creditLine}}</span>
					    		...
					    		<template>
								    <el-table
								      ...
								      style="width: 100%">
								        ...
								        label="已上传的内容">
								        <template slot-scope="scope">
								        	<div v-if="scope.row.content" v-for="(item,contentIndex) in scope.row.content">
    ...
    <el-dialog
  		title="选择担保人"
  		width="450px"
  		:visible.sync="dialogSelectGuarantorForm">
		  <template>
...
//      activeVerifyFirmPlatformNames:'FirmFinancialPlatformOne',//公司管理情况
        ...
        supplyDataData:{
        	items:[{item:'',itemText:'',content:[{url:'',fileName:'123412',id:''}],upload:''}]
        },//影响资料数据
        merchantCode:'',//商户code
        ...
        locationTypeList:'',//下拉列表-经营地下拉信息查询
			  ...
			  capitalTypeSeletList:'',//补充资料_企业基本信息_股东名单_类型下拉
			  changeTypeSeletList:'',//补充资料_企业基本信息_股东变更_变更事项下拉
...
//      } else
    ...
    methods: {
    	tirggerFile (event,type,index) {//上传影像
			  var file = event.target.files;
			  let formData = new FormData();
			  formData.append('file',file[0])
			  formData.append('type',type)
			  formData.append('supplierCapitalId',this.supplierCapitalId)
    	...
    	resetFaceCheckTimes(){//重置人脸识别次数
    		enterpriseData.resetFaceCheckTimes(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	this.$message.success(res.data.message)
          } else{
          	this.$message.error(res.data.message)
    	...
    	handleClick(tab){//融资概况，基本信息，担保信息，渠道信息，授信管理,合同管理
    		if(tab.index==='0'){
...
//					this.getAssetSubHouseTypeList();//获取下拉列表-资产子类型-房
    		...
    		if(tab.index==='3'){
    			this.getQueryCompanyManage();//补充资料-公司管理情况-公司管理情况查询------------查询
    		}else
    		if(tab.index==='4'){//关联公司状态
          this.getQueryAffiliatedCompany();// 补充资料-关联公司状态-关联公司状态------查询
//					this.getAffiliateTypeList();//获取下拉列表-与借款企业关系
    		}else
    		if(tab.index==='5'){
    			this.getQueryMarketInfo();//补充资料-主营业务及市场查询------------查询
    		}else
    		if(tab.index==='6'){
    			this.queryIntoUpdownstream();//上下游数据
    		}else
    		if(tab.index==='7'){
    			this.getQueryBorrowInfo();//补充资料-借款信息-借款信息查询------------查询
    		}else if(tab.index==='8'){
...
//  			this.getQueryGrade();//授信签署-评级-查询评级
    	...
    	getRemitView(){
    		enterpriseData.remitView(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
    	...
    	remitConfirm(){
    		enterpriseData.remitConfirm(this.supplierCapitalId).then(res => {
	          if (res.data.code === '200') {
	          	this.bankAmt = res.data.data.bankAmt;
	          	this.getRemitView();
	          	this.$message.success(res.data.message)
...
//      this.getQueryReview();//获取操作项
      ...
      getFinanceGeneralSituation () {//获取融资概况
      	enterpriseData.financeGeneralSituation(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
...
//			  this.getCapitalTypeSeletList();//获取补充资料_企业基本信息_股东名单_类型下拉
//			  this.getChangeTypeTypeSeletList();//获取补充资料_企业基本信息_股东变更_变更事项下拉
      },
      getQueryCustomType (){//尽调信息-客户类型查询------------查询
      	enterpriseData.customType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      getQueryEnterpriseInfo (){//尽调信息-基本情况详情------------查询
      	enterpriseData.basicData(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveEnterpriseInfo (){//尽调信息-基本情况修改
      	...
      	enterpriseData.saveBasicData(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      saveBorrowInfo (){//尽调信息-基本情况修改
      	this.$message.success('操作成功！')
      },
      getQueryAccountLicense(){//尽调信息-开户许可证详情------------查询
      	enterpriseData.accountLicense(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveAccountLicense (){//尽调信息-开户许可证修改
      	...
      	data.supplierCapitalId = this.supplierCapitalId
      	enterpriseData.saveAccountLicense(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryExtendInfo(){//尽调信息-补充资料-企业基本信息_扩展信息(发展, 背景, 往来)查询------------查询
      	enterpriseData.extendInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveExtendInfo (){//尽调信息-补充资料-企业基本信息_扩展信息保存
      	let data = baseUtil.copy(this.extendInfoData)
      	data.supplierCapitalId = this.supplierCapitalId
      	enterpriseData.saveExtendInfo(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryCompanyManage(){//补充资料-公司管理情况-公司管理情况查询------------查询
      	enterpriseData.companyManage(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveCompanyManage (){//补充资料-公司管理情况-公司管理情况查询------保存
      	let data = baseUtil.copy(this.companyManageData)
      	data.supplierCapitalId = this.supplierCapitalId
      	enterpriseData.updateCompanyManage(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryMarketInfo(){//补充资料-主营业务及市场查询------------查询
      	enterpriseData.marketInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveMarketInfo (){//补充资料-主营业务及市场保存------保存
      	let data = baseUtil.copy(this.marketInfoData)
      	data.supplierCapitalId = this.supplierCapitalId
      	enterpriseData.saveMarketInfo(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryBorrowInfo(){//补充资料-借款信息-借款信息查询------------查询
      	enterpriseData.borrowInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      queryIntoUpdownstream(){//-上下游数据-进入上下游页面---查询
      	enterpriseData.intoUpdownstream(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      queryStreamData(){//失去焦点查询
      	...
      	enterpriseData.queryTitanUpdownstream(this.merchantCode).then(res => {
          if (res.data.code === '200') {
          	this.titanUpdownstreamData = res.data.data
          } else {
            this.$message.error(res.data.message)
      ...
      saveQueryTitanUpdownstream (){//补充资料-上下游数据查询按钮------保存
      	if(!this.merchantCode){
      		return
      	}
      	let data = {
      	...
      	enterpriseData.saveUpdownstreamData(data).then(res => {
          if (res.data.code === '200') {
          	this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryFileList(){//补充资料-影像列表-查询------------查询
      	enterpriseData.fileList(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      getQueryReview(){
      	enterpriseData.getQueryReview(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      getQuerySummary () {
        enterpriseData.headData(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
...
//      } else
      ...
      doInspectionRelateShow (type,bool) { //
        ...
        enterpriseData[api](this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      doCentreListShow(){
      	enterpriseData.centreList(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      createFactorContract  (){//生成保理合同
      	enterpriseData.createFactorContract(this.supplierCapitalId).then(res => {
	          if (res.data.code === '200') {
	          	this.$message.success(res.data.message)
...
//          }
            this.$confirm(`<p style="font-size: 14px;padding: 10px;text-align: center">确认通过审核?</p>`, '确认通过审核', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              dangerouslyUseHTMLString: true
            }).then(() => {
...
//		          }
//		        }).catch(() => {})
						enterpriseData.supplySubmitCheck(this.supplierCapitalId).then(res => {
		          if (res.data.code === '200') {
		            this.$message.success(res.data.message)
		            this.rejectDialogShow = false
		            this.getQuerySummary ();//头部信息
      ...
      doReject(){//点击拒绝
        ...
        enterpriseData.reject(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
            this.rejectDialogShow = false
            this.getQuerySummary ();//头部信息
      ...
      getQueryBasicData () {
        enterpriseData.queryBasic(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
...
//    getQueryTradeData () {
//      enterpriseData.queryTrade(this.supplierCapitalId).then(res => {
//        if (res.data.code === '200') {
...
//    }, // 贸易资料
//    getQueryKnowData() {
//    		enterpriseData.queryKnow(this.supplierCapitalId).then(res => {
//    					if(res.data.code ==='200') {
...
//			  this.getLevelInfoSeletList();//获取下拉列表-严重程度
			...
			getAgentInfoSeletList (){//获取下拉列表-分销商下拉信息查询
				enterpriseData.agentInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getLocationTypeList (){//获取下拉列表-企业基本信息_经营地下拉
				enterpriseData.locationType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getMemberTypeSeletList (){//获取下拉列表-分销商下拉信息查询
				enterpriseData.memberType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getAssetSubHouseTypeList (){//获取下拉列表-资产子类型-房
				enterpriseData.assetSubHouseType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getAssetSubCarTypeList (){//获取下拉列表-资产子类型-车
				enterpriseData.assetSubCarType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getAffiliateTypeList (){//获取下拉列表-与借款企业关系
				enterpriseData.affiliateType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getLevelInfoSeletList (){//获取下拉列表-分销商下拉信息查询
				enterpriseData.levelInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getBankListSeletList (){//获取下拉列表-分销商下拉信息查询
				enterpriseData.bankList(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getCertificateTypeSeletList (){//获取下拉列表-证件类型下拉信息查询
				enterpriseData.certificateType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          ...
          if (res.data.code === '200') {
			...
			getCapitalTypeSeletList (){//获取补充资料_企业基本信息_股东名单_类型下拉
				enterpriseData.capitalType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
			...
			getChangeTypeTypeSeletList (){//获取补充资料_企业基本信息_股东变更_变更事项下拉
				enterpriseData.changeType(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      saveEquity (){//尽调信息-股东名单------------保存
      	...
      	enterpriseData.saveShareHolding(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryOffice(){//尽调信息-股东变更信息列表查询------------查询
      	enterpriseData.enterpriseChange(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0&&this.queryOfficeData.length===0){
          		this.queryOfficeData.push({
			      		type:'',before:'',after:'',changeDate:''
			      	})
          	}else{
      ...
      saveOffice (){//尽调信息-股东变更信息列表查询------------保存
      	...
      	enterpriseData.saveEnterpriseChange(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryAffiliatedCompany(){// 补充资料-关联公司状态-关联公司状态------查询
        enterpriseData.affiliatedCompany(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0){
          		this.affiliatedCompanyData=[];
          		this.affiliatedCompanyData.push({
			      		affiliateType:'',affiliateTypeText:'',enterpriseName:'',enterpriseController:'',mainBusiness:''
			      	})
          	}else{
      ...
      deleteAffiliatedCompany (index){//补充资料-关联公司状态-关联公司状态------------删除
      	this.affiliatedCompanyData.splice(index,1)
      },
      plusAffiliatedCompany (){//补充资料-关联公司状态-关联公司状态------------添加
      	this.affiliatedCompanyData.push({
      		affiliateType:'',affiliateTypeText:'',enterpriseName:'',enterpriseController:'',mainBusiness:''
      	})
      },
      saveAffiliatedCompany (){//补充资料-公司管理情况-公司管理情况查询--------保存
        let info = baseUtil.copy(this.affiliatedCompanyData)
      	...
      	enterpriseData.updateAffiliatedCompany(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
     	...
     	getQueryManager(){//尽调信息-补充资料-财产信息-财产信息查询------------查询
      	enterpriseData.assetInfo(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	...
          	if(res.data.data.houseInfo.length===0){
          		...
          		this.queryHouseInfoData.push({
			      		id:'',assetSubType:'',assetSubTypeText:'',estimatedValue:'',paymentMonthly:'',assetName:''
          		})
      ...
      saveManager (){//尽调信息-补充资料-财产信息-------------保存
      	...
      	enterpriseData.updateAsset(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryCredit(){//尽调信息-征信记录列表查询------------查询
      	enterpriseData.queryCredit(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0){
          		this.queryCreditData = [];
          		this.queryCreditData.push({
			      		type:'',typeText:'',versionType:'',versionTypeList:'',versionTypeText:'',belongName:'',queryDate:'',abnormalCount:'',queryTimes:'',desc:'',
			      	})
      ...
      creditTypeChange(index,val){//征信记录 下拉类型改变
      	...
      	enterpriseData.creditBelong(data).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length!==0){
      ...
      saveCredit (){//尽调信息-征信记录列表查询------------保存
      	...
      	enterpriseData.saveCredit(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      getQueryPaper(){//尽调信息-企业附件列表查询------------查询
      	enterpriseData.queryPaper(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0){
          		this.queryPaperData = [];
          		this.queryPaperData.push({
			      		type:'',typeText:'',num:'',startDate:'',endDate:'',status:'',approvalStatus:'',approvalStatusText:'',uploadDate:'',files:[],
			      	})
          	}else{
      ...
      savePaper (){//尽调信息-企业附件列表查询------------保存
      	...
      	enterpriseData.savePaper(data).then(res => {
          if (res.data.code === '200') {
          	this.getQueryPaper();//尽调信息-企业附件列表查询
            this.$message.success(res.data.message)
          } else {
          ...
          if (res.data.code === '200') {
          	if(res.data.data.length===0&&this.queryEnterpriseOpinionData.length===0){
          		this.queryEnterpriseOpinionData.push({
			      		opinionDate:'',opinionDescription:'',severityType:'',
			      	})
          	}else{
      ...
      getQueryEnterpriseGuarantor(){//尽调信息--法人信息-查询------------查询
      	enterpriseData.queryEnterpriseGuarantor(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
...
//				}
				data.supplierCapitalId =this.supplierCapitalId
      	enterpriseData.saveEnterpriseGuarantor(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
          } else {
            this.$message.error(res.data.message)
      ...
      SelectGuarantor(index){//点击选择触发
      	...
      	enterpriseData.selectGuarantorList(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0){
      ...
      saveSelectGuarantor(){//保存选择的担保人
      	...
      	enterpriseData.createTestKey(guarantorId).then(res => {
          if (res.data.code === '200') {
      ...
      doMoreGuaranteeFile(index){ // 尽调企业 担保人新增附件
      	this.queryEnterpriseGuarantorData[index].guaranteeValueFile.push({key:'',url:''})
      },
...
//    	this.queryEnterpriseGuarantorData.splice(index,1)
          ...
          enterpriseData.deleteEnterpriseGuarantor(guarantorId).then(res => {
	          if (res.data.code === '200') {
      ...
      plusEnterpriseGuarantor (){//尽调信息-担保人------------新增
      	this.queryEnterpriseGuarantorData.push({
      		guarantorId:'',name:'',address:'',identityNo:'',mobile:'',email:'',identityFrontKey:'',identityFrontUrl:'',identityBackKey:'',identityBackUrl:'',guaranteeValue:'',guaranteeValueFile:[]
      	})
      },
      getQueryDistributor(){//尽调信息-平台信息-查询------------查询
      	enterpriseData.queryDistributor(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data.length===0&&this.queryDistributorData.length===0){
          		this.queryDistributorData.push({
			      		selectable:'able',centreId:'',name:'',username:'',password:'',companyName:'',businessLicense:'',contractName:'',contractNum:'',validDate:'',bankCode:'',bankBranch:'',bankAccountNum:'',files:[],supplierCapitalId:'',
			      	})
          	}else{
      ...
      deleteDistributor (id){//尽调信息-平台信息-删除------------删除
      	...
      	enterpriseData.deleteDistributor(data).then(res => {
          if (res.data.code === '200') {
//        	this.queryDistributorData.splice(index,1)
      ...
      saveDistributor (item){//尽调信息-平台信息-保存------------保存
      	...
      	data.files = JSON.stringify(data.files)
				...
				data.supplierCapitalId =this.supplierCapitalId
      	enterpriseData.operateDistributor(data).then(res => {
          if (res.data.code === '200') {
          	this.getQueryDistributor();//尽调信息-平台信息-查询------------查询
            this.$message.success(res.data.message)
          } else {
      ...
      getContractSignNameList (centreId){
      	enterpriseData.contractSignNameList(centreId).then(res => {
          if (res.data.code === '200') {
...
//	      	}
      ...
      getQuerySignData () {
        enterpriseData.querySign(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      changeRate(loanRate){//获取年利率，月利率
      	enterpriseData.calculatedInterest(loanRate).then(res => {
          if (res.data.code === '200') {
            this.rate.monthRate = res.data.data.monthRate
            this.rate.yearRate = res.data.data.yearRate
          } else {
      ...
      getContractInfoData () {// 查询合同信息
        enterpriseData.contractList(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      getQueryGrade () {
        enterpriseData.queryGrade(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data){
      ...
      getQueryEvaluateGrade () {
        enterpriseData.evaluateGrade(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
          	if(res.data.data){
          		this.queryGradeData = res.data.data
          		this.$message.success(res.data.message)
      ...
      getQueryGradeDetail () {
        enterpriseData.queryGradeDetail(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      doTradeRmSummary() {
        enterpriseData.queryTradeRmSummary(this.supplierCapitalId).then(res => {
      ...
      doRefreshTrade(){
      	enterpriseData.refreshTrade(this.supplierCapitalId).then(res => {
          if (res.data.code === '200') {
      ...
      showPdf(type){
      	...
      	if(type == 'entrepreneur_people_bank'){//企业人行
      		enterpriseData.creditEnterprise(this.supplierCapitalId).then(res => {
		        if (res.data.code === '200') {
      	...
      	if(type == 'corporate_people_bank'){//法人人行
      		enterpriseData.creditRepre(this.supplierCapitalId).then(res => {
		        if (res.data.code === '200') {
		        	this.$showPDF(res.data.data.fileUrl);
		        } else {
		          this.$message.error(res.data.message)
      	...
      	if(type == 'underwriter_people_bank'){//担保人人行
      		enterpriseData.creditGuarantor(this.supplierCapitalId).then(res => {
		        if (res.data.code === '200') {
		        	this.$showPDF(res.data.data.fileUrl);
		        } else {
		          this.$message.error(res.data.message)
      ...
      doSaveApproveTrade (index) {
        ...
        code = code.trim()
        if (code) {
          enterpriseData.saveApproveTrade(this.supplierCapitalId, code).then(res => {
            if (res.data.code === '200') {
              this.$message.success(res.data.message)
              this.tradeRmSummaryData = []
              this.tradeVendorCodeData = []
      ...
      doVerifyRmSummary() {
        enterpriseData.queryVerifyRmSummary(this.supplierCapitalId).then(res => {
...
//          }
      ...
      doVerifyRmDetail(type) {
        enterpriseData.queryVerifyRmDetail(this.supplierCapitalId, type).then(res => {
