            ...
            <div class="tab-content">
              ...
              <div class="apply-info info-list" v-if=false> <!--暂时隐藏-->
                ...
                <template v-if="queryBasicData.registrant">
                  ...
                  <p><span>身份证：</span><img  alt="正面" :src='queryBasicData.registrant.idcardPicFront'><img alt="反面" :src='queryBasicData.registrant.idcardPicBack'></p>
                </template>
              </div>
              ...
              <div class="enterprise-info info-list">
                ...
                <template v-if="queryBasicData.businessLicense">
                  ...
                  <p class="img_bgc"><span >营业执照：</span><img @click="lookImg(queryBasicData.businessLicense.pic)"  alt="营业执照" :src='queryBasicData.businessLicense.pic'></p>
                </template>
              </div>
              <div class="legal-representative-info info-list">
                ...
                <template v-if="queryBasicData.repre">
                  ...
                  <p class="img_bgc"><span>法定代表人身份证：</span><img @click="lookImg(queryBasicData.repre.idcardPicFront)" alt="正面" :src='queryBasicData.repre.idcardPicFront'><img @click="lookImg(queryBasicData.repre.idcardPicBack)" alt="反面" :src='queryBasicData.repre.idcardPicBack'></p>
                </template>
              </div>
              <div class="info-list">
                ...
                <template v-if="queryBasicData.bondsman">
                  ...
                  <p class="img_bgc"><span>担保人身份证：</span><img @click="lookImg(queryBasicData.bondsman.idcardPicFront)" alt="正面" :src='queryBasicData.bondsman.idcardPicFront'><img @click="lookImg(queryBasicData.bondsman.idcardPicBack)" alt="反面" :src='queryBasicData.bondsman.idcardPicBack'></p>
                </template>
              </div>
              <div class="info-list">
                ...
                <template v-if="queryBasicData.account">
                  ...
                  <p class="img_bgc"><span>开户许可证：</span>
                  	<img @click="lookImg(queryBasicData.account.pic)" alt="正面" :src='queryBasicData.account.pic'>
                  </p>
                </template>
              </div>
          ...
          <el-tab-pane label="贸易资料" name="second">
            <div class="tab-content">
              ...
              <div class="info-list" v-for="(item,index) in queryTradeData" :key="index">
                ...
                <p><span class="width-100">交易平台编码：</span>{{item.scmVendorCode}}</p>
                ...
                <div class="info-list">
                  <p class="title">征信数据</p>
                    ...
                    <Upload 
                    	...
                    	actionUrl="/capital-api/supplier_register/upload_verify" 
	                  ...
	                  <el-form-item label="实际结算金额（近六月回款银行金额）：">
	                    <div class="input-group">
	                      <div v-for="(bankStatementsItem, bankStatementsIndex) in queryVerifyData.centres[index].bankStatements" :key="bankStatementsIndex" class="input-group-item">
	                        ...
	                        <el-input style="width:120px;" v-model="queryVerifyData.centres[index].bankStatements[bankStatementsIndex]"></el-input>
	                      </div>
	                    </div>
	                  </el-form-item>
	                  ...
	                  <el-form-item label="实际开票金额（近六月开票记录）：">
	                    <div class="input-group">
	                      <div v-for="(taxStatementsItem, taxStatementsIndex) in queryVerifyData.centres[index].taxStatements" :key="taxStatementsIndex" class="input-group-item">
	                        ...
	                        <el-input style="width:120px" v-model="queryVerifyData.centres[index].taxStatements[taxStatementsIndex]"></el-input>
	                      </div>
	                    </div>
	                	</el-form-item>
                  </div>
                  ...
                  <table class="">
                    ...
                    <tr v-for="(item, index) in querySignData.creditAndIterestRates">
                      <td style="width: 120px;">
                        <el-input v-model="querySignData.creditAndIterestRates[index].enterpriseName" style="width: 90%" size="mini" clearable  :disabled="true">
                        </el-input>
                      </td>
                      <td>
                      ...
                      <td>
                        <el-input-number class="must_input" v-model="querySignData.creditAndIterestRates[index].interestRate" :min="0.05" :max="0.065" :precision="3" :step="0.001" :controls="false" size="mini">

                        </el-input-number>
                        <span class="left_label">每日</span>
                      </td>
                      <td>
                        <el-input-number class="must_input" v-model="querySignData.creditAndIterestRates[index].overdueRate" :min="0.08" :max="0.09" :precision="3" :step="0.001"  :controls="false" size="mini" >

                        </el-input-number>
                        <span class="left_label">每日</span>
                      </td>
                      <td>
                        <el-input-number class="must_input" v-model="querySignData.creditAndIterestRates[index].chargeRate"  :min="0" :max="2" :precision="2" :step="0.1" :controls="false" size="mini" >

                        </el-input-number>
                        <span class="left_label">每笔</span>
                      </td>
                      <td>
                        <el-input-number class="must_input" v-model="querySignData.creditAndIterestRates[index].marginRate"  :min="0.5" :max="2" :precision="2" :step="0.1" :controls="false" size="mini" >
          ...
          <template v-else>
            ...
            <li v-if="registerStage === 'SIGN'" >
              ...
              <div>
                ...
                <el-button v-if="roleHasPermissions['supplier_manage_register_list_review']" type="primary" @click="doApproveFinal(true)">入驻成功</el-button>
              </div>
            </li>
          </template>
...
//      this.getQuerySignData();//授信签署
      ...
      createGuaranteeContract(){//生成保证合同
      	supplierDetail.createGuaranteeContract(this.supplierId).then(res => {
	          if (res.data.code === '200') {
	          	this.$message.success(res.data.message)
	          	this.doInspectionRelateShow('contractStatus');
	          } else {
