  ...
  <div class="page-container" ref="page-container" id="page-container"><!---还款列表--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="还款日期（起~止）:"  prop="searchTime">
        	<template>
					  <div class="block">
					    <el-date-picker
					      v-model="searchTime"
    ...
    <el-table :data="list" @current-change="handleSelectionChange" highlight-current-row :max-height=tableMaxHeight>
		...
		<el-table-column prop="financeNo" label="融资单号/期数" min-width="100">
			<template slot-scope="scope">
				<div>{{scope.row.financeNo}}</div>
				<div>第{{scope.row.periods}}/{{scope.row.totalPeriods}}期</div>
			</template>
		</el-table-column>
		<el-table-column prop="supplierName" label="商户" width="220">
			<template slot-scope="scope">
		...
		<el-table-column prop="planAmt" label="计划还款金额" min-width="80" align="center">
		</el-table-column>
		
		<el-table-column prop="havePlanedAmt" label="已还金额" min-width="80" align="center">
		</el-table-column>
		
		<el-table-column prop="balanceAmt" label="剩余应还金额" min-width="80" align="center">
		</el-table-column>
		
		<el-table-column prop="planDate" label="计划还款时间" min-width="80" align="center">
		</el-table-column>
		...
		<el-table-column prop="backPlanNo" label="操作" width="122" align="center">
			<template slot-scope="scope" v-if="!roleHasPermissions['payment_manage_payment_list_view']">
				<router-link :to="'repaymentDetail/' + scope.row.backPlanNo"  class="router-link">查看</router-link>
			</template>
		</el-table-column>
