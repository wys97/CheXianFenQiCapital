  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="法定代表人手机" prop="businessLicenseNo">
          <el-input v-model="listQuery.businessLicenseNo" style="width:130px;"></el-input>
        </el-form-item>
        ...
        <el-form-item label="审核状态" class="query-select" prop="approveStatus">
          <el-select v-model="listQuery.approveStatus" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
        ...
        <!--<el-form-item label="进件状态" class="query-select" prop="approveStatus">
          <el-select v-model="listQuery.approveStatus" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        prop="creditLineMonth"
        label="建议期限期限（月）">
      </el-table-column>

      <el-table-column
        prop="capitalCreditLine"
        label="资方授信额度（万元）">
      </el-table-column>
      
      <el-table-column
        prop="capitalCreditLineMonth"
        label="资方授信期限（月）">
      </el-table-column>
      ...
      <el-table-column
        ...
        label="审核状态">
        <template slot-scope="scope" >
          <span>{{scope.row.approveStatusText}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
        		<!--<router-link tag="a" target="_blank" :to="'/enterpriseDataManage/enterpriseData/'+scope.row.id" class="router-link">
			        	企业资料 
			      </router-link>-->
			      <router-link :to="'merchantAuditDetail/' + scope.row.id" class="router-link">详情</router-link>
            <router-link :to="'chinaTravelBankDetail/' + scope.row.id" class="router-link">授信报告</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,getRegisterStatusList,checkStatusList,finallyStatusList} from '@/api/merchantAuditManage/merchantAuditList'
