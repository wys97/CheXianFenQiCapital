	...
	<div>
		<el-form ref="verifyFirmFinancialTrideForm" label-width="180px" size="mini" label-position="right">
			<div class="info-list">
				...
				<el-form-item label="所属资方：" width="180px">
					...
					<el-input style="width:200px;" :value="capitalEnterpriseList.capitalName" :label="capitalEnterpriseList.capitalName" class="must_input"></el-input>
				</el-form-item>
...
<script>
	...
	import { queryCapitalEnterprise,queryEnterprise} from '@/api/common'
...
//				this.getQueryEnterpriseNameList();//企业名称模糊查询
			...
			getQueryEnterpriseList(){//商户信息-信息下拉
				queryEnterprise().then(res => {
					if(res.data.code === '200') {
						this.enterpriseList = res.data.data.supplierInfos;
					} else {
						this.$message.error(res.data.message)
					}
				}).catch(() => {})
			...
			getQueryCapitalEnterpriseList(){//资方信息-信息列表下拉
				queryCapitalEnterprise().then(res => {
					if(res.data.code === '200') {
						this.capitalEnterpriseList = res.data.data.capitalInfo
					} else {
						this.$message.error(res.data.message)
					}
				}).catch(() => {})
