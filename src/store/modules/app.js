...
const app = {
  ...
  mutations: {
  	setEnterpriseTopData:(state,id)=>{
				enterpriseData.detailTop(id).then(res => {
          if (res.data.code === '200') {
          	state.enterpriseTopData=res.data.data;
          } else{
    ...
    ADD_VISITED_VIEWS: (state, view) => {
      ...
      if (view.name === '供应商详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '供应商详情' })
        if (findIndex === -1) {
      ...
      if (view.name === '商户审核详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '商户审核详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '融资放款详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '融资放款详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '放款详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '放款详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '逾期详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '逾期详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
  ...
  actions: {
  	getEnterpriseTopData({commit},id){
  		commit('setEnterpriseTopData',id)
  	},
